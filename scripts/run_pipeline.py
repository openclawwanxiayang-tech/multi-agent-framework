...
def init_task(task_id: str, title: str) -> Path:
    ...
    append_event(task_dir, {
        ...
        "timestamp": utc_now(), "stage": "SPEC", "event": "TASK_CREATED", "actor": "system",
        "details": {"mode": "pipeline", "risk_tier": "low"},
    })
    return task_dir
...
def transition(task_id: str, to_stage: str, owner_id: str = "orchestrator") -> None:
    ...
    try:
        ...
        if validator:
            rc = os.system(f"python3 {ROOT / validator} {task_dir} >/dev/null")
            if rc != 0:
                raise RuntimeError(f"Validator failed for stage {from_stage}")
        ...
        append_event(task_dir, {
            ...
            "timestamp": utc_now(), "stage": to_stage,
            "event": "TASK_COMPLETED" if to_stage == "DONE" else "STAGE_TRANSITION",
            "actor": owner_id, "details": {"from": from_stage, "to": to_stage},
        })
    finally:
        lock.release()
